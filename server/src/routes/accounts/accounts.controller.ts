...
import { getWarehouseById } from "routes/warehouses/warehouse.service";
...
export const getUserInfo = async (
  ...
  try {
    ...
    // 如果用户有绑定仓库 ID，则获取对应仓库信息
    if (account?.warehouseID) {
      warehouse = await getWarehouseById(account.warehouseID);
    }

    res.json({
      ...account,
      warehouse, // 添加 warehouse 信息
    });
  } catch (error) {
